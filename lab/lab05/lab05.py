...
def add_trees(t1, t2):
    ...
    if len(t1)>=len(t2):
        return tree(label(t1)+label(t2),[add_trees(t1[i],t2[i]) for i in range (1,len(t2))]+[t1[i] for i in range(len(t2),len(t1))])
    return tree(label(t1)+label(t2),[add_trees(t1[i],t2[i]) for i in range (1,len(t1))]+[t2[i] for i in range(len(t1),len(t2))])
