...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        first=t.branches[0].label
        second=t.branches[1].label
        if first<=second:
            t.branches.pop(1)
...
def num_splits(s, d):
    ...
    def all_set(s):
        if not s:
            ...
            return [[i[0]+s[0],i[1]] for i in all_set(s[1:])]+[[i[0],i[1]+s[0]] for i in all_set(s[1:])]
    result=all_set(s)
    count=0
    for i in result:
        if abs(i[0]-i[1])<=d:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            edits=skel_char+helper_align(skeleton_idx+1,code_idx+1)[0]
            cost=helper_align(skeleton_idx+1,code_idx+1)[1]
            possibilities.append((edits,cost))
        else:
            # Insert
            fast=helper_align(skeleton_idx,code_idx+1)
            edits=''.join(['+['+code_char+']'+fast[0]])
            cost=1+fast[1]
            possibilities.append((edits, cost))
            # Delete
            fast=helper_align(skeleton_idx+1,code_idx)
            edits=''.join(['-['+skel_char+']'+fast[0]])
            cost=1+fast[1]
            possibilities.append((edits , cost))
