...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        pos=self.place  # REPLACE THIS LINE
        count=0
        while not pos.is_hive():
            if pos.bees and self.min_range<=count<=self.max_range:
                return bee_selector(pos.bees)
            pos=pos.entrance
            count+=1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for i in bees:
            index=self.place.bees.index(i)
            self.place.bees[index].reduce_health(reflected_damage)
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.chewing>0:
            ...
            food=bee_selector(self.place.bees)
            if food:
                food.reduce_health(food.health)
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        while pos:
            if pos.ant:
                pos.ant.buff()
            if pos.ant and pos.ant.is_container():
                if pos.ant.contained_ant:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if self.is_scared and not self.place.entrance.is_hive():
            destination=self.place.entrance
        if self.is_scared and self.place.entrance.is_hive():
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while  pos:
            if not pos.is_hive():
                for i in pos.bees:
                    insects_in_front.update({i:distance})
                if pos.ant and distance>0:
                    insects_in_front.update({pos.ant:distance})
                    if pos.ant.is_container() and distance>0:
                        if pos.ant.contained_ant:
                            insects_in_front.update({pos.ant.contained_ant:distance})
            distance+=1
            pos=pos.entrance
        return insects_in_front
        # END Problem Optional 3

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 3
        damage=2
        damage=damage-1/16*self.insects_shot
        if distance*0.25>damage:
            return 0
        else:
            return damage-distance*0.25
