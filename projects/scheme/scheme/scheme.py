...
def scheme_eval(expr, env,_=None): # Optional third argument is ignored
    """Evaluate Scheme expression EXPR in environment ENV. Don't change the expr.
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operator=first
        operands=rest.map(lambda x:x)
        temp=operands
        if type(operator)==MacroProcedure:
            return scheme_eval(operator.apply_macro(operands,env),env)
        while temp!=nil:
            temp.first=scheme_eval(temp.first,env)
            temp=temp.rest
        return scheme_apply(operator,operands,env)
        operator_function, operands = scheme_eval(first, env), rest.map(lambda x: scheme_eval(x, env))
        validate_procedure(operator_function)
        return scheme_apply(operator_function, operands, env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target,scheme_eval(second,env))
        return target
        # END PROBLEM 5
...
def do_or_form(expressions, env):
    ...
    if len(expressions)==0:
        return False
    value=scheme_eval(expressions.first,env)
    while is_false_primitive(value):
        expressions=expressions.rest
        if expressions.rest==nil:
            value=scheme_eval(expressions.first,env,True)
            break
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        if expressions.rest==nil:
            raise SchemeError('No body function')
